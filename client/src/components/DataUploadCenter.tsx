...
import { AlertTriangle, ArrowRight, CheckCircle2, CloudUpload, Download, FileCheck2, FileSpreadsheet, RefreshCw, Trash2, X } from "lucide-react";
import * as XLSX from "xlsx";
import { autoMapColumns, departmentImportFields, mapSpreadsheetRows, missingRequiredFields, templateRows, type ColumnMapping, type SpreadsheetRow } from "@shared/dataMapping";
...
function readWorkbook(file: File, departmentId: string, onComplete: (workbook: PendingWorkbook) => void, onError: (message: string) => void) {
  ...
  reader.onload = (event) => {
    try {
      ...
      onComplete({ departmentId, fileName: file.name, size: file.size, sheetName, headers, rows, mappings: autoMapColumns(fields, headers) });
    } catch (error) {
...
export default function DataUploadCenter({ uploads, setUploads }: { uploads: UploadMap; setUploads: React.Dispatch<React.SetStateAction<UploadMap>> }) {
  ...
  const updateMapping = (fieldId: string, sourceHeader: string) => {
    setPendingWorkbook((current) => {
      ...
      if (sourceHeader === "__unmapped__") delete next[fieldId];
      else next[fieldId] = sourceHeader;
      return { ...current, mappings: next };
    });
  };
  ...
  const handleFile = (departmentId: string, file?: File) => {
    ...
    readWorkbook(file, departmentId, (workbook) => {
  ...
  const confirmImport = () => {
    ...
    notify(`${pendingWorkbook.fileName} was accepted with ${Object.keys(pendingWorkbook.mappings).length} mapped fields.`);
