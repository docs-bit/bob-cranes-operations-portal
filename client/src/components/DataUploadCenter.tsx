...
export default function DataUploadCenter({ uploads, setUploads }: { uploads: UploadMap; setUploads: React.Dispatch<React.SetStateAction<UploadMap>> }) {
  ...
  }} style={{ height: 28, fontSize: 11 }}><Download size={12} /> Export flagged rows CSV ({rowValidationErrors.length})</button>}</div><table style={{ width: "100%", minWidth: 720, borderCollapse: "collapse", fontSize: 12 }}><thead><tr>{pendingWorkbook.headers.map(header => <th key={header} style={{ textAlign: "left", padding: "9px 10px", borderBottom: "2px solid var(--border, #cbd5e1)", background: "var(--muted, #f8fafc)", whiteSpace: "nowrap" }}>{header}</th>)}</tr></thead><tbody>{pendingWorkbook.rows.slice(0, 20).map((row, rowIndex) => {
