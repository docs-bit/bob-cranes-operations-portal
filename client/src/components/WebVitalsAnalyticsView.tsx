...
export default function WebVitalsAnalyticsView() {
  ...
  const lcpValues = metrics.filter(e => e.metricName === "LCP").map(e => Number.parseFloat(e.metricValue)).filter(v => !Number.isNaN(v));
  const fidValues = metrics.filter(e => e.metricName === "FID").map(e => Number.parseFloat(e.metricValue)).filter(v => !Number.isNaN(v));
  const clsValues = metrics.filter(e => e.metricName === "CLS").map(e => Number.parseFloat(e.metricValue)).filter(v => !Number.isNaN(v));
      ...
      <section className="panel" style={{ marginTop: 20 }}>
        <div className="panel-header">
          <div>
            ...
            <div className="panel-meta">Showing dedicated metric entries captured from real browser sessions.</div>
          </div>
