...
export default function SalesEnquiryInbox({
  ...
  const downloadQuoteBrief = async () => {
    ...
    try {
      ...
      const filename = await generateRentalQuotePdf({
        enquiry: selected,
        generatedBy: actor.role === "admin" ? "Administrator" : "Sales workspace",
      });
