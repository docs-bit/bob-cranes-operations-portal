...
import { CheckCircle2, Edit3, KeyRound, Power, Search, ShieldCheck, UserPlus, UsersRound, X, Activity, AlertTriangle } from "lucide-react";
...
export default function DepartmentUsersView() {
  ...
  return <div className="content">
    ...
    <section className="panel user-activity-panel"><div className="panel-header"><div><div className="panel-title"><Activity size={16} /> Recent account activity</div><div className="panel-meta">Sign-ins, profile changes, and access status changes for local department accounts.</div></div><span className="status-badge blue">{activity.length} events</span></div><div className="panel-body activity-list">{activityQuery.isLoading ? <div className="empty-state">Loading activity…</div> : activityQuery.error ? <div className="account-error">Unable to load account activity.</div> : activity.length ? activity.slice(0, 8).map((event) => <div className="activity-row" key={event.id}><div className="activity-icon"><Activity size={14} /></div><div className="activity-copy"><div><strong>{event.userName ?? event.userEmail ?? "Unknown user"}</strong><span className="activity-action">{activityLabel(event.action)}</span></div><p>{event.detail}</p></div><time>{displayDateTime(event.createdAt)}</time></div>) : <div className="empty-state">No account activity has been recorded yet.</div>}</div></section>
