...
export default function Home() {
  ...
  const advanceDepartmentBooking = async (booking: Booking, config: any) => {
    const nextStage = booking.stage === config.secondaryStage ? config.secondaryNextStage : config.nextStage;
    if (!nextStage) return;
    try {
  ...
  const completeDepartmentWorkstream = async (booking: Booking, config: any) => {
    ...
    try {
      ...
      setCompletedWorkstreams(current => ({
        ...current,
        [booking.id]: Array.from(new Set([...(current[booking.id] ?? []), config.parallelCode!])),
      }));
